...
from ui.components.custom_dialog import show_info, show_warning, show_error, show_question, show_success
...
from .encoder import _check_gpu_encoder_available, _ensure_even_resolution, RealtimeEncodingLogger
...
from core.video.CreateFinalVideo import _rescale_tts_metadata_to_duration, _update_tts_metadata_path
    ...
    try:
        ...
        while app.batch_processing:
            ...
            for url in app.url_queue:
                if (url not in processed_urls and
                    app.url_status.get(url) in ['waiting', None]):
            ...
            if not waiting_urls:
                ...
                for _ in range(10):
                    ...
                    time.sleep(1)
                    new_waiting = [
                        url for url in app.url_queue
                        if url not in processed_urls
                        and app.url_status.get(url) in ['waiting', None]
                    ]
                    if new_waiting:
                        app.add_log(f"대기열에서 새 URL {len(new_waiting)}개를 감지했습니다. 이어서 처리합니다.")
                        found = True
                        break
            ...
            while retry_count < max_retries and app.batch_processing:
                try:
                    ...
                    if retry_count == 0:
                        ...
                        app.add_log(f"[Batch] 재시도 {retry_count}/{max_retries} 진행 중: {url[:50]}...")
                    ...
                    if _safe_get_url_status(app, url) == 'skipped':
                        app.add_log(f"[SKIP] [{current_index}/{total_in_queue}] 건너뜀 - 다음 영상으로 진행")
                    ...
                    try:
                        ...
                        logger.info("[LocalSave] 저장 시작 - generated_videos: %d개", len(getattr(app, 'generated_videos', [])))
                        ...
                        try:
                            if hasattr(app, 'output_manager') and app.output_manager:
                                verification_result = app.output_manager.verify_video_log(url)
                                app.url_remarks[url] = verification_result
                                logger.debug("[비고] 로그 검증 결과: %s", verification_result)
                        except Exception as verify_err:
                            logger.warning("[비고] 로그 검증 실패: %s", verify_err)
                            app.url_remarks[url] = "통과"

                    except Exception as e:
                        logger.error("[LocalSave] Failed to move generated videos: %s", e, exc_info=True)
                    ...
                    if "503" in error_msg or "overloaded" in error_lower or "unavailable" in error_lower:
                        wait_minutes = 5
                        app.add_log(f"[WARN] ⏸️ Gemini 서버 과부하 감지! {wait_minutes}분 후 재시도합니다...")
                        ...
                        for remaining in range(wait_minutes * 60, 0, -1):
                            if not app.batch_processing:
                                ...
                                break
                            if remaining % 60 == 0:
                                app.add_log(f"[INFO] 대기 중... {remaining // 60}분 남음")
                            time.sleep(1)
                    ...
                    elif "429" in error_msg or "quota" in error_lower or "resource_exhausted" in error_lower:
                        retry_count += 1
                        if retry_count < max_retries:
                            wait_time = retry_count * 15  # 15, 30, 45초
                            app.add_log(f"[WARN] API 할당량 초과. {wait_time}초 후 다른 키로 재시도...")
                            ...
                            app.add_log(f"❌ {max_retries}번 재시도 실패 (모든 API 키 소진)")
                    ...
                    # ★ 500 기타 서버 오류 → 1분 대기 후 재시도
                        ...
                        if retry_count < max_retries:
                            wait_time = 60  # 1분 대기
                            app.add_log(f"[WARN] 서버 오류 발생. {wait_time}초 후 재시도...")
                        ...
                        if any(token in lowered for token in permission_indicators):
                            app.add_log("[WARN] API 권한 오류 감지. 현재 키를 차단하고 다른 키로 교체합니다.")
                            try:
                                # 장기간 차단하여 곧바로 재사용되지 않도록 처리
                                app.api_key_manager.block_current_key(duration_minutes=60)
                            ...
                            try:
                                ...
                                app.add_log(f"[WARN] 권한 오류 후 새 API 키 확보 실패: {switch_exc}")
                        _safe_set_url_status(app, url, 'failed')
                        # 비고란에 짧은 오류 메시지 저장
                        app.url_status_message[url] = _get_short_error_message(e)
                        failed_count += 1
            ...
            try:
                ...
                logger.debug("[정리] 임시 파일 정리 실패 (무시됨): %s", str(cleanup_err)[:50])
            ...
            if app.batch_processing:
                pending = [
                    candidate for candidate in app.url_queue
        ...
        if pending_remaining:
            app.add_log(f"미처리 URL {len(pending_remaining)}건은 대기 상태로 남아 있습니다.")
        app.add_log("=" * 60)
...
def _process_single_video(app, url, current_number, total_urls):
    ...
    if url in app.url_timestamps:
        ...
        logger.debug("[폴더 관리] 기존 타임스탬프 재사용: %s", app._processing_start_time.strftime('%Y%m%d_%H%M%S'))
    else:
        # 새로운 URL이면 새 타임스탬프 생성
        app._processing_start_time = datetime.now()
        app.url_timestamps[url] = app._processing_start_time
        logger.debug("[폴더 관리] 새 타임스탬프 생성: %s", app._processing_start_time.strftime('%Y%m%d_%H%M%S'))
    ...
    try:
        ...
        app.update_progress_state('download', 'processing', 5, "현재 원본 동영상을 찾고 있습니다.")
        app.update_step_progress('download', 20)
        app.add_log(f"📥 [{current_number}/{total_urls}] 영상 다운로드 중...")

        # 저장 폴더 변경 시나 기존 다운로드 파일이 없으면 재다운로드
        need_redownload = (
            not hasattr(app, '_temp_downloaded_file') or
            app._temp_downloaded_file is None or
            not os.path.exists(app._temp_downloaded_file)
        )

        if need_redownload:
            downloaded_path = DouyinExtract.download_tiktok_douyin_video(url)
            app._temp_downloaded_file = downloaded_path
            app.add_log(f"[다운로드] 새로 다운로드 완료: {os.path.basename(downloaded_path)}")
        else:
            app.add_log(f"[다운로드] 기존 파일 사용: {os.path.basename(app._temp_downloaded_file)}")
        ...
        if original_video_duration > MAX_VIDEO_DURATION:
            ...
            app.add_log(f"⏭️ [{current_number}/{total_urls}] {skip_message} - 다음 영상으로 자동 이동")
        ...
        app.update_progress_state('analysis', 'processing', 5, "동영상을 분석하고 있습니다.")
        app.update_step_progress('analysis', 20)
        app.add_log(f"[AI] [{current_number}/{total_urls}] 영상 분석 중... ({original_video_duration:.1f}s)")
        ...
        app.update_progress_state('translation', 'processing', 5, "번역과 각색을 하고 있습니다.")
        ...
        if selected_voices:
            ...
            # 선택된 음성이 없으면 기본값 사용
            voices = getattr(app, "multi_voice_presets", getattr(app, "available_tts_voices", [app.fixed_tts_voice]))

        voice_labels = [_get_voice_display_name(v) for v in (voices if isinstance(voices, list) else [voices])]
        if selected_voices:
            logger.info("[음성 선택] 사용자가 선택한 음성 사용: %s", ', '.join(voice_labels))
        else:
        ...
        for idx_voice, voice in enumerate(voices, 1):
            ...
            logger.debug("[자막 캐시] 음성 %d/%d - 전체 TTS 데이터 초기화 (새로 계산)", idx_voice, total_voices)
            ...
            app.update_progress_state('tts', 'processing', voice_progress, f"{voice_label} 음성 합성 중입니다.")
            app.update_step_progress('tts', 20)
            tts_sync_progress = max(5, int(((idx_voice - 1) / total_voices) * 100))
            app.update_progress_state('tts_audio', 'processing', tts_sync_progress, f"{voice_label} 음성 생성 준비 중입니다.")
            _generate_tts_for_batch(app, voice)
            after_voice_progress = max(voice_progress, int((idx_voice / total_voices) * 100))
            app.update_progress_state('tts', 'processing', after_voice_progress, f"{voice_label} 음성 합성이 완료되었습니다.")
            app.update_step_progress('tts', int(20 + 80 * idx_voice / total_voices))
            tts_complete_progress = max(tts_sync_progress, int((idx_voice / total_voices) * 100))
            app.update_progress_state('tts_audio', 'processing', tts_complete_progress, f"{voice_label} 음성 생성 완료! 싱크 계산을 준비합니다.")
            if idx_voice == total_voices:
                app.update_progress_state('tts_audio', 'completed', 100, "모든 음성 생성이 끝났습니다.")
            ...
            app.update_progress_state('video', 'processing', video_progress, f"{voice_label} 음성으로 영상을 준비 중입니다.")
            app.update_step_progress('video', 20)
            _create_final_video_for_batch(app, voice, idx_voice, total_voices, current_number, total_urls)
            after_video_progress = max(video_progress, int((idx_voice / total_voices) * 100))
            app.update_progress_state('video', 'processing', after_video_progress, f"{voice_label} 음성으로 렌더링 중입니다.")
            app.update_step_progress('video', int(20 + 80 * idx_voice / total_voices))

        app.update_progress_state('tts', 'completed', 100, f"음성 합성 단계가 완료되었습니다. (작업 {current_number}/{total_urls})")
        app.update_progress_state('video', 'completed', 100, f"영상 렌더링 단계가 완료되었습니다. (작업 {current_number}/{total_urls})")

        # URL 1개 처리 완료 - 비용 요약 출력
        app.token_calculator.log_session_summary(f"영상 완성 [{current_number}/{total_urls}]")
        # 다음 URL을 위해 비용 초기화
        ...
        is_api_recoverable = any(token in error_lower for token in [
...
def _create_final_video_for_batch(app, voice, voice_index=None, voice_total=None, job_index=None, total_jobs=None):
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("[배치 비디오 생성] %s", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        logger.info("=" * 60)
        ...
        app.update_progress_state('audio_merge', 'processing', merge_start, progress_msg)
        ...
        cached_last_frame = None  # 마지막 프레임 캐시 (블러 처리된 VideoClip의 파일 참조 문제 방지)
        try:
            ...
            try:
                ...
                logger.debug("중국어 자막 제거 완료 (프레임 캐시 실패): %s", frame_cache_err)
        except Exception as e:
            ui_controller.write_error_log(e)
        ...
        if not combined_audio_path or not os.path.exists(combined_audio_path):
            app.update_progress_state('audio_merge', 'error', merge_start, "TTS 오디오 결합에 실패했습니다.")
            raise Exception("TTS 파일 결합 실패")

        app.update_progress_state('audio_merge', 'processing', merge_end, "오디오 합본 완료! 싱크 분석으로 넘어갑니다.")
        ...
        if actual_audio_end and actual_audio_end > 0:
            ...
            logger.info("[동기화] Gemini 분석 기준 실제 음성 길이 사용: %.2f초", actual_tts_duration)
        else:
        ...
        logger.info("  TTS 오디오: %.3f초 (speed_ratio: %sx)", actual_tts_duration, sync_info.get('speed_ratio', 1.0))
        ...
        if shortage > original_duration * 0.5:  # guard against extremely long TTS vs video
            logger.warning("TTS is %.1fs longer than video (%.0f%%)", shortage, (actual_tts_duration/original_duration*100))
        ...
        if real_audio_dur + eps < desired_cut:
            ...
            padded_path = os.path.join(app.tts_output_dir, "speeded_tts_padded_tail.wav")
            ...
            logger.debug("  [Audio] Added %.2fs silence -> %.3fs (path only, timing preserved)", pad_ms/1000, real_audio_dur)
        ...
        # 2) Audio longer than target -> trim slightly
            ...
            logger.debug("  [Audio] Trimmed to %.3fs (offset: %.3fs maintained)", real_audio_dur, current_offset)
        ...
        if hasattr(app, '_per_line_tts') and app._per_line_tts:
            for entry in app._per_line_tts:
                if isinstance(entry, dict):
                    ...
                    if isinstance(end_time, (int, float)) and end_time > last_subtitle_end:
        ...
        logger.info("    Target video duration: %.3fs (콘텐츠 + 1.0s)", target_video_duration)
        ...
        if video.duration + eps < target_video_duration:
            ...
            if cached_last_frame is not None:
                try:
                    ...
                    logger.debug("  [Video] Extended (cached frame): %.3fs -> %.3fs", original_duration, video.duration)
                except Exception as cache_err:
                    logger.debug("  [Video] 캐시 프레임 사용 실패: %s", str(cache_err)[:30])
            ...
            if cached_last_frame is None:
                try:
                    ...
                    tail_frame.fps = original_fps
                    video = concatenate_videoclips([video, tail_frame])
                    logger.debug("  [Video] Extended: %.3fs -> %.3fs (freeze frame)", original_duration, video.duration)
                except Exception as frame_err:
                    # 3순위: 원본 비디오에서 직접 가져오기
                    logger.debug("  [Video] 프레임 추출 실패, 원본에서 재시도: %s", str(frame_err)[:40])
                    try:
                        source_video_clip = VideoFileClip(source_video)
                        frame_time = max(min(source_video_clip.duration - 0.01, video.duration - 0.01), 0)
                        last_frame_array = source_video_clip.get_frame(frame_time)
                        tail_frame = ImageClip(last_frame_array, duration=extend_dur)
                        tail_frame.fps = original_fps
                        video = concatenate_videoclips([video, tail_frame])
                        source_video_clip.close()
                        logger.debug("  [Video] Extended (from source): %.3fs -> %.3fs", original_duration, video.duration)
                    except Exception as fallback_err:
                        # 최후의 수단: 비디오 연장 포기
                        logger.debug("  [Video] 연장 실패, 현재 길이 유지: %.3fs", video.duration)
        elif video.duration > target_video_duration + eps:
            video = video.subclipped(0, target_video_duration)
            logger.debug("  [Video] Trimmed: %.3fs -> %.3fs", original_duration, video.duration)
        ...
        if app.add_subtitles.get() if hasattr(app, 'add_subtitles') else True:
            try:
                if hasattr(app, '_cached_subtitle_clips') and app._cached_subtitle_clips is not None:
                    ...
                    logger.debug("=" * 70)
                    logger.debug("  target_video_duration: %.3fs", target_video_duration)
                    logger.debug("  real_audio_dur: %.3fs", real_audio_dur)
                    sync_info = getattr(app, 'tts_sync_info', {}) or {}
                    logger.debug("  tts_sync_info:")
                    logger.debug("    - timestamps_source: %s", sync_info.get('timestamps_source', 'unknown'))
                    logger.debug("    - speeded_duration: %s", sync_info.get('speeded_duration', 'N/A'))
                    logger.debug("    - audio_start_offset: %s", sync_info.get('audio_start_offset', 'N/A'))
                    logger.debug("    - actual_audio_end: %s", sync_info.get('actual_audio_end', 'N/A'))
                    logger.debug("=" * 70)

                    if voice_index and voice_total:
                        analysis_message = f"[{voice_index}/{voice_total}] {voice_label} - computing subtitle timing from 1.2x audio"
                    else:
                        analysis_message = f"{voice_label} - computing subtitle timing from 1.2x audio"
                    app.update_progress_state('audio_analysis', 'processing', analysis_progress_base, analysis_message)
                    # _extend_last_subtitle_to_video_end 호출 제거: Gemini 타임스탬프 존중
                    # 이전 코드는 마지막 자막을 영상 끝까지 강제 연장해서 싱크가 깨졌음
                    subtitle_clips = create_subtitle_clips_for_speed(app, target_video_duration)
                    app._cached_subtitle_clips = subtitle_clips

                subtitle_count = len(subtitle_clips) if subtitle_clips else 0
                if subtitle_count:
                    analysis_message = f"Subtitles synced: {subtitle_count} timing points"
                else:
                    analysis_message = "Audio analysis finished; using default timing."
                app.update_progress_state('audio_analysis', 'processing', analysis_progress_base, analysis_message)
                if not voice_total or (voice_index and voice_total and voice_index == voice_total):
                    app.update_progress_state('audio_analysis', 'completed', 100, analysis_message)
                app.update_progress_state('subtitle_overlay', 'processing', overlay_progress_base, "Overlaying subtitles with calculated timings.")
                ...
                if subtitle_clips and len(subtitle_clips) > 0:
                    ...
                    overlay_message = "No subtitles generated; continuing without burn-in."
                app.update_progress_state('subtitle_overlay', 'processing', overlay_progress_base, overlay_message)
                if not voice_total or (voice_index and voice_total and voice_index == voice_total):
                    app.update_progress_state('subtitle_overlay', 'completed', 100, overlay_message)
            except Exception as e:
                ui_controller.write_error_log(e)
                logger.warning("  Subtitle error: %s", str(e))
                overlay_message = "An error occurred while applying subtitles."
                app.update_progress_state('subtitle_overlay', 'error', overlay_progress_base, "An error occurred while applying subtitles.")
        else:
            analysis_message = "Subtitles disabled; skipping audio analysis for captions."
        ...
        if final_video.duration > final_cut_point:
            logger.debug("  Trimming: %.3fs -> %.3fs", final_video.duration, final_cut_point)
            final_video = final_video.subclipped(0, final_cut_point)
        elif final_video.duration + eps < final_cut_point:
            logger.debug("  Video short; adjusting duration: %.3fs -> %.3fs", final_video.duration, final_cut_point)
        ...
        app.update_progress_state('post_tasks', 'processing', polish_progress, "영상 마무리 작업으로 싱크를 한번 더 점검 중입니다.")
        if not voice_total or (voice_index and voice_total and voice_index == voice_total):
            app.update_progress_state('post_tasks', 'completed', 100, "잔여 작업 정리 완료! 마무리 단계로 이동합니다.")
        ...
        if voice_index and voice_total:
            ...
            finalize_message = f"{voice_label} - 피날레 렌더링으로 완성본을 출력 중입니다."
        app.update_progress_state('finalize', 'processing', finalize_progress, finalize_message)
        ...
        cpu_ffmpeg_params = [
            "-profile:v", "baseline",
            "-level", "4.2",  # 1920x1080 30fps 지원 (level 3.1은 720p까지만 지원)
            *common_ffmpeg_params,
        ]
        ...
        if hasattr(app, 'register_generated_video'):
            app.register_generated_video(voice, output_path, final_duration, file_size, temp_dir)
        if voice_index and voice_total:
            render_complete_msg = f"[{voice_index}/{voice_total}] {voice_label} 렌더링 완료! 결과물을 정리하고 있습니다."
        else:
            render_complete_msg = f"{voice_label} 렌더링 완료! 결과물을 정리하고 있습니다."
        app.update_progress_state('finalize', 'processing', 100, render_complete_msg)

        if not voice_total or (voice_index and voice_total and voice_index == voice_total):
            app.update_progress_state('audio_merge', 'completed', 100, "모든 오디오 합본이 마무리되었습니다.")
            app.update_progress_state('audio_analysis', 'completed', 100, analysis_message if 'analysis_message' in locals() else "오디오 분석 완료")
            app.update_progress_state('subtitle_overlay', 'completed', 100, overlay_message if 'overlay_message' in locals() else "자막 적용 단계를 마쳤습니다.")
            app.update_progress_state('post_tasks', 'completed', 100, "잔여 작업 정리 완료! 마무리 단계로 이동합니다.")
            app.update_progress_state('finalize', 'completed', 100, f"{voice_label} 음성 기반 영상이 준비되었습니다{job_suffix}.")
...
def clear_all_previous_results(app):
    ...
    # ★ 개별 오디오 파일 유지 - 사용자 요청 ★
    if hasattr(app, '_per_line_tts') and app._per_line_tts:
        kept_count = sum(1 for tts_data in app._per_line_tts
                        if isinstance(tts_data, dict) and 'path' in tts_data
                        and tts_data['path'] and os.path.exists(tts_data['path']))
        if kept_count > 0:
        ...
        if hasattr(app, 'base_tts_dir') and os.path.exists(app.base_tts_dir):
            ...
            for session_dir in os.listdir(app.base_tts_dir):
                if session_dir.startswith("session_"):
                    ...
                    if session_path != app.tts_output_dir and os.path.isdir(session_path):
                        dir_time = os.path.getmtime(session_path)
                        if (current_time - datetime.fromtimestamp(dir_time)).total_seconds() > 3600:
                            try:
                                shutil.rmtree(session_path)
                                logger.debug("[초기화] 오래된 세션 삭제: %s", session_dir)
                            except Exception as cleanup_err:
                                logger.debug("[초기화] 세션 삭제 실패 (무시됨): %s - %s", session_dir, str(cleanup_err)[:50])
    except Exception as e:
    ...
    for step in app.progress_states:
        app.progress_states[step] = {'status': 'waiting', 'progress': 0, 'message': None}
    ...
    if hasattr(app, 'root'):
        ...
        if hasattr(app, 'script_text'):
            ...
            app.root.after(0, lambda: app.script_text.insert(1.0, "새로운 동영상 분석을 시작합니다..."))

        if hasattr(app, 'translation_text'):
            app.root.after(0, lambda: app.translation_text.delete(1.0, tk.END))
            app.root.after(0, lambda: app.translation_text.insert(1.0, "한국어 번역 중..."))

        if hasattr(app, 'tts_result_text'):
            app.root.after(0, lambda: app.tts_result_text.delete(1.0, tk.END))
            app.root.after(0, lambda: app.tts_result_text.insert(1.0, "TTS 음성 생성 대기 중..."))
