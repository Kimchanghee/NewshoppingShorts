...
def create_final_video_thread(app):
    ...
    try:
            ...
            logger.debug("[Video] Resolution: %dx%d", app.cached_video_width, app.cached_video_height)
            ...
            logger.debug("[AudioSync] Target video duration: %.2fs (TTS + 3.0s 안전)", target_video_duration)
            ...
            if original_duration > target_video_duration:
                ...
                logger.debug("[Timing] Video longer by %.1fs - trimming video to %.2fs", original_duration - target_video_duration, target_video_duration)

                video = video.subclipped(0, target_video_duration)

                logger.debug("[AudioSync] Video trimmed to %.2fs to match TTS + 3.0s", target_video_duration)

            elif original_duration < target_video_duration:

                logger.debug("[Timing] Video shorter by %.1fs - extending video to %.2fs", target_video_duration - original_duration, target_video_duration)
                ...
                logger.debug("[AudioSync] Video extended to %.2fs with freeze frame", target_video_duration)
            ...
            if app.add_subtitles.get():
                ...
                try:
                    ...
                    if subtitle_clips:
                        ...
                        logger.debug("[Subtitles] Last subtitle end: %.2fs", last_subtitle_end)
            ...
            if watermark_enabled and watermark_channel_name:
                ...
                logger.info("[워터마크] 적용 중: '%s' at %s (%dx%d)", watermark_channel_name, watermark_position, video_w, video_h)
            ...
            if subtitle_applied and last_subtitle_end > 0:
                ...
                if final_video.duration > last_subtitle_end + 2.0:
                    ...
                    logger.debug("[Trim] Using subtitle end: %.1fs -> %.1fs", final_video.duration, cut_point)

                    final_video = final_video.subclipped(0, cut_point)

                    logger.debug("[Trim] Completed subtitle-based cut: %.1fs", final_video.duration)

                else:

                    logger.debug("[Trim] No trim needed - video already ends at %.1fs", final_video.duration)
                ...
                if final_video.duration > audio_duration + 2.5:
                    ...
                    logger.debug("[Trim] Using audio duration: %.1fs -> %.1fs", final_video.duration, cut_point)

                    final_video = final_video.subclipped(0, cut_point)

                    logger.debug("[Trim] Completed audio-based cut: %.1fs", final_video.duration)
            ...
            logger.info("[Complete] Temp file created: %s (%.1f MB)", output_path, file_size)
            ...
            if validation_enabled and os.path.exists(output_path):
                ...
                try:
                    ...
                    if validation_passed:
                        ...
                        logger.warning("[영상 검증] 영상은 저장되었습니다. 검증 결과를 확인하세요.")
        ...
        try:
            ...
            if "length_adjusted" in filename:
                ...
                for tts_data in app._per_line_tts:
                    ...
                    logger.debug("[TTS 議곗젙] 二쇱쓽: ?먮낯??李얠쓣 ???놁뼱 議곗젙???뚯씪 ?ъ슜")
            ...
            logger.debug("[TTS Adjust] Original length: %.1fs, target: %.1fs", audio_duration, target_duration)
            ...
            if abs(audio_duration - target_duration) <= 1.0:
                ...
                logger.debug("[TTS 議곗젙] 湲몄씠媛 ?곸젅??(李⑥씠 %s珥?, 洹몃?濡??ъ슜", abs(audio_duration - target_duration))
            ...
            if audio_duration > target_duration:
                ...
                logger.debug("[TTS Adjust] Speed needed: %.2fx to %.1f sec", speed_ratio, target_duration_with_margin)
                ...
                if speed_ratio <= 1.5:
                    ...
                    logger.debug("[TTS 議곗젙] 諛⑸쾿: 以묎컙 ?덉쭏 諛곗냽 (frame rate 議곗젙)")
                    ...
                    logger.debug("[TTS 議곗젙] 寃쎄퀬: %s諛곗냽? ?덈Т 鍮좊쫫, 2諛곗냽?쇰줈 ?쒗븳", speed_ratio)
                ...
                logger.debug("[TTS Adjust] Speed result: %.1fs -> %.1fs", audio_duration, adjusted_duration)
            ...
            if os.path.exists(adjusted_path):
                ...
                logger.debug("[TTS Adjust] Final verify: %.1f sec (target: %.1f sec)", final_duration, target_duration)
...
def _extend_last_subtitle_to_video_end(app, video_duration):
    ...
    logger.warning("[WARNING] _extend_last_subtitle_to_video_end is deprecated and should not be called")
    ...
    if original_start >= video_duration:
        ...
        logger.debug("[Subtitles] Adjusted last subtitle start from %.2fs to %.2fs (was beyond video end)", original_start, last_entry['start'])



    last_entry["end"] = video_duration

    logger.debug("[Subtitles] Set last subtitle end to %.2fs (was %.2fs) - CTA complete display guaranteed", video_duration, original_end)
...
def create_subtitle_clips_improved(app, video_duration):
    ...
    try:
        ...
        if timed_segments and _metadata_segments_meaningful(timed_segments, video_duration):
            ...
            logger.info("[Subtitles] Using %d timed segments for burn-in", len(timed_segments))
            ...
            for idx_preview, (start_ts, duration, text) in enumerate(timed_segments[:min(5, len(timed_segments))], start=1):
                ...
                logger.debug("[Subtitles] Seg[%d] %.2f-%.2fs :: %s", idx_preview, start_ts, end_ts, sample)
            ...
            logger.warning("[Subtitles] Timed metadata insufficient (%d segment), falling back to translation text", len(timed_segments))
        ...
        logger.debug("[Subtitles] Translation fallback segments: %d", len(fallback_segments))

        if not fallback_segments:

            logger.warning("[Subtitles] Translation text missing, skipping subtitle generation.")
...
def _resolve_video_dimensions(app):
    ...
    if source_video and os.path.exists(source_video):
        ...
        try:
            ...
            logger.debug("[Subtitles] Measured dimensions %dx%d from source", video_width, video_height)
    ...
    logger.debug("[Subtitles] Cached dimensions set to %dx%d", video_width, video_height)
        ...
        for i, entry in enumerate(per_line):
            ...
            if isinstance(entry, dict):
                ...
                if isinstance(start, (int, float)) and isinstance(end, (int, float)):
                    ...
                    logger.debug("    #%d: %.3fs - %.3fs (%.3fs) '%s'", i+1, start, end, duration, text_preview)

                else:

                    logger.debug("    #%d: start=%s, end=%s '%s'", i+1, start, end, text_preview)
    ...
    if per_line:
        ...
        for idx, item in enumerate(per_line, start=1):
            ...
            if not isinstance(item, dict):
                ...
                logger.debug("[Subtitles Skip] Seg #%d: Not a dict (type=%s)", idx, type(item).__name__)
            ...
            if not text or not isinstance(start, (int, float)) or not isinstance(end, (int, float)):
                ...
                logger.debug("[Subtitles Skip] Seg #%d: Missing/invalid fields: %s", idx, ", ".join(missing_fields))
            ...
            if end <= start:
                ...
                if idx == len(per_line):
                    ...
                    logger.debug("[Subtitles Adjust] Last segment had non-positive duration -> %.2f-%.2fs", safe_start, safe_end)
                    ...
                    logger.debug("[Subtitles Skip] Seg #%d: Invalid timing (start=%.2fs >= end=%.2fs) - '%s'", idx, start, end, text[:20])
            ...
            if start_ts >= video_duration + 5.0:  # 5초 이상 초과만 skip (CTA 완전 보호)
                ...
                logger.debug("[Subtitles Skip] Seg #%d: Starts too late (%.2fs > video_duration %.2fs + 5.0s) - '%s'", idx, start_ts, video_duration, text[:20])
            ...
            if len(segments) < 3 or len(segments) >= len(per_line) - 3:
                ...
                logger.debug("[Subtitle Timing] Seg #%d: %.2fs-%.2fs (%.2fs) '%s'", len(segments)+1, start_ts, end_ts, actual_duration, text[:20])
            ...
            if start_ts + actual_duration > video_duration:
                ...
                logger.debug("[Subtitle Trim] Seg #%d trimmed: %.2fs -> %.2fs (video ends at %.2fs)", len(segments)+1, actual_duration, duration, video_duration)
            ...
            if duration < 0.05:  # 0.1에서 0.05로 완화
                ...
                logger.debug("[Subtitles Skip] Seg #%d: Too short (%.2fs < 0.05s) - '%s'", idx, duration, text[:20])
            ...
            if segment_key in seen_segments:
                ...
                logger.debug("[Subtitles Skip] Seg #%d: Duplicate (start=%.2fs, text='%s')", idx, start_ts, text[:20])
        ...
        if segments:
            ...
            for i, (start, dur, text) in enumerate(segments):
                ...
                logger.debug("    #%d: %.3fs - %.3fs (%.3fs) '%s'", i+1, start, end, dur, text_preview)
            ...
            logger.warning("[Subtitles] No valid timed segments extracted (skipped %d, duplicates %d)", skipped, duplicates)
...
def _build_analysis_based_segments(app, video_duration):
    ...
    if segments:
        ...
        logger.info("[Subtitles] Built %d segments from analysis timestamps", len(segments))
...
def _extract_translation_segments(app, desired_count=None, max_chars=20):
    ...
    if hasattr(app, '_per_line_tts') and app._per_line_tts and isinstance(app._per_line_tts, list):
        ...
        if tts_segments:
            ...
            logger.debug("[Subtitles] Using TTS segmentation: %d segments (sync preserved)", len(tts_segments))
...
def _split_subtitle_with_gemini(app, text: str, max_chars: int = 10) -> list:
    ...
    try:
        ...
        if segments:
            ...
            logger.debug("[Subtitle Split] Gemini 분할: '%s...' -> %d개", text[:20], len(segments))
