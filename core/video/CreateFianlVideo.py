...
def create_final_video_thread(app):
    try:
            # 鍮꾨뵒???앹꽦 ?④퀎 ?쒖옉
            app.update_progress_state('video', 'processing', 0, "영상을 최종 인코딩 하고 있습니다.")
            app.update_overall_progress('video', 'processing', 0)
            
            app.update_status("理쒖쥌 鍮꾨뵒???앹꽦 以?..")
            
            selected_voice = app.fixed_tts_voice
            logger.info("[理쒖쥌 鍮꾨뵒???앹꽦] ?쒖옉")
            logger.info("?먮낯 鍮꾨뵒?? {app.source_video}")
            logger.info("TTS ?뚯씪 ?? {len(app._per_line_tts)}")
            logger.info("?좏깮??TTS ?뚯꽦: {selected_voice}")
            logger.info("醫뚯슦 諛섏쟾 ?곸슜: {app.mirror_video.get()}")
            logger.info("?먮쭑 ?앹꽦: {app.add_subtitles.get()}")
            ...
            logger.info("[Video] Resolution: %sx%s", app.cached_video_width, app.cached_video_height)
            original_duration = video.duration
            video_duration = original_duration
            original_fps = video.fps
            logger.info("[Video] Duration %.2fs, FPS %s", original_duration, original_fps)
            app.update_progress_state('video', 'processing', 20)
            
            # 醫뚯슦 諛섏쟾 泥섎━
            if app.mirror_video.get():
                logger.info("[鍮꾨뵒??泥섎━] 醫뚯슦 諛섏쟾 ?곸슜")
                video = video.fx(vfx.mirror_x)
            else:
                logger.info("[鍮꾨뵒??泥섎━] 醫뚯슦 諛섏쟾 誘몄쟻??(?먮낯 ?좎?)")
            
            # 以묎뎅???먮쭑 ?쒓굅 泥섎━try:
            try:
                video = app.apply_chinese_subtitle_removal(video)
                logger.debug("Chinese subtitle removal completed")
            except Exception as e:
                logger.debug("Chinese subtitle removal failed: %s", e)
            ...
            logger.info("[AudioSync] TTS duration: %.2fs", audio_duration)
            logger.info("[AudioSync] Target video duration: %.2fs (TTS + 3.0s safe)", target_video_duration)
            logger.info("[AudioSync] Original video duration: %.2fs", original_duration)

            app.update_progress_state('video', 'processing', 50)

            if original_duration > target_video_duration:
                logger.info("[Timing] Video longer by %.1fs - trimming video to %.2fs", original_duration - target_video_duration, target_video_duration)
                video = video.subclipped(0, target_video_duration)
                logger.info("[AudioSync] Video trimmed to %.2fs to match TTS + 3.0s", target_video_duration)
            elif original_duration < target_video_duration:
                logger.info("[Timing] Video shorter by %.1fs - extending video to %.2fs", target_video_duration - original_duration, target_video_duration)
                # concatenate_videoclips already imported at top level

                extension_duration = target_video_duration - original_duration
                last_frame = video.to_ImageClip(t=video.duration - 0.01, duration=extension_duration)
                video = concatenate_videoclips([video, last_frame])
                logger.info("[AudioSync] Video extended to %.2fs with freeze frame", target_video_duration)
            else:
                logger.info("[Timing] Video duration already matches TTS + 3.0s")
            ...
            if app.add_subtitles.get():
                ...
                try:
                    ...
                    if subtitle_clips:
                        for idx_preview, clip in enumerate(subtitle_clips[:min(5, len(subtitle_clips))], start=1):
                            start_ts = getattr(clip, "start", 0.0)
                            duration = getattr(clip, "duration", 0.0)
                            logger.debug("[Subtitles] Clip %d: %.2f-%.2fs", idx_preview, start_ts, start_ts + duration)
                        ...
                        logger.info("[Subtitles] Last subtitle end: {last_subtitle_end:.2f}s")
                        ...
                        logger.info("[鍮꾨뵒??泥섎━] ?앹꽦???먮쭑???놁쓬")
                except Exception as e:
                    ui_controller.write_error_log(e)

            else:
                logger.info("[鍮꾨뵒??泥섎━] ?먮쭑 ?앹꽦 嫄대꼫?")
            
            # ?곸긽 ???먮Ⅴ湲?- CTA 완전 표시를 위해 여유 시간 확대
            if subtitle_applied and last_subtitle_end > 0:
                ...
                if final_video.duration > last_subtitle_end + 2.0:
                    ...
                    logger.info("[Trim] Using subtitle end: %.1fs -> %.1fs", final_video.duration, cut_point)
                    final_video = final_video.subclipped(0, cut_point)
                    logger.info("[Trim] Completed subtitle-based cut: %.1fs", final_video.duration)
                else:
                    logger.info("[Trim] No trim needed - video already ends at %.1fs", final_video.duration)
            else:
                # ?ㅻ뵒??湲곗??쇰줈 ?먮Ⅴ湲?(?먮쭑???녿뒗 寃쎌슦)
                if final_video.duration > audio_duration + 2.5:
                    cut_point = audio_duration + 2.5
                    logger.info("[Trim] Using audio duration: %.1fs -> %.1fs", final_video.duration, cut_point)
                    final_video = final_video.subclipped(0, cut_point)
                    logger.info("[Trim] Completed audio-based cut: %.1fs", final_video.duration)
            ...
            logger.info("[Output] Temporary export path: %s", output_path)

            app.update_progress_state('video', 'processing', 80)

            logger.info("[?몄퐫?? 理쒖쥌 ?곸긽 ?뚮뜑留??쒖옉")
            # 임시 오디오 파일 경로 (절대 경로 사용)
            temp_audio_path = os.path.join(temp_dir, "temp-audio.m4a")
            # MP4 최대 호환성을 위한 필수 옵션
            safe_ffmpeg_params = [
                "-profile:v", "high",      # 4K 지원을 위한 High 프로파일
                "-level", "5.1",           # 4K 60fps까지 지원 (최대 호환성)
            ...
            logger.info("[?꾨즺] ?꾩떆 ?뚯씪 ?앹꽦: {output_path} ({file_size:.1f} MB)")
            logger.info("[?꾨즺] 理쒖쥌 湲몄씠: {final_duration:.1f}s")
            ...
            try:
                subprocess.run(
                    [
                        ...
                        output_path,
                        '/inheritance:e',
                        '/grant', '*S-1-1-0:(R)',       # Everyone
                        '/grant', '*S-1-5-32-545:(R)',  # Users group
                    ],
                    check=True,
                    capture_output=True,
                    creationflags=subprocess.CREATE_NO_WINDOW
                )
                logger.debug("Permission setting: read permission added")
            except (OSError, subprocess.SubprocessError) as e:
                logger.debug("Permission setting failed (ignored): %s", e)
                ui_controller.write_error_log(e)
            ...
            try:
                ...
                logger.debug("Failed to remove combined audio: %s", e)
            ...
            if validation_enabled and os.path.exists(output_path):
                ...
                app.update_progress_state('video', 'processing', 95, "영상 품질을 검증하고 있습니다.")

                try:
                    validator = VideoValidationManager(app)
                    validation_passed, final_path, validation_result = validator.validate_and_fix(
                        output_path,
                        auto_fix=getattr(app, 'auto_fix_video', False)  # 자동 수정은 기본 비활성화
                    )
                    ...
                    if validation_passed:
                        logger.info("[Video Validation] Quality check passed")
                    else:
                        # 검증 실패해도 영상은 저장 (사용자가 확인 가능하도록)
                        vr = validation_result.get('validation_result', validation_result)
                        score = vr.get('score', 0)
                        logger.warning("[Video Validation] Quality check failed (score: %d/100)", score)
                        logger.info("[Video Validation] Video saved. Please check validation results.")
                ...
                except (ValueError, RuntimeError, IOError) as val_error:
                    # 검증 실패해도 영상 생성은 성공으로 처리
                    logger.warning("[Video Validation] Error during validation (ignored): %s", str(val_error))
        ...
        try:
            ...
            logger.debug("Failed to close video: %s", close_err)
        try:
            if 'new_audio' in dir() and new_audio is not None:
                new_audio.close()
        except (OSError, RuntimeError) as close_err:
            logger.debug("Failed to close audio: %s", close_err)
        try:
            if 'final_video' in dir() and final_video is not None:
                final_video.close()
        except (OSError, RuntimeError) as close_err:
            logger.debug("Failed to close final video: %s", close_err)
...
def combine_tts_files(app, target_duration=None):
        ...
        try:
            ...
            if not os.path.exists(tts_path):
                logger.info("[TTS ?뚯씪] 議댁옱?섏? ?딆쓬: {tts_path}")
                return None

            # Gemini Audio Understanding이 이미 정확한 타임스탬프 제공 - 추가 조정 불필요
            logger.info("[TTS 파일] Gemini 타임스탬프 사용 - 메타데이터 조정 없음")

            # ???대? 議곗젙???뚯씪?몄? ?뺤씤 ??
            filename = os.path.basename(tts_path)
            if "length_adjusted" in filename:
                logger.info("[TTS 議곗젙] 寃쎄퀬: ?대? 議곗젙???뚯씪?낅땲?? {tts_path}")
                ...
                for tts_data in app._per_line_tts:
                    if tts_data['path'] and "full_script_tts" in tts_data['path']:
                        ...
                        logger.info("[TTS 議곗젙] ?먮낯 ?뚯씪 ?ъ슜: {tts_path}")
                        _update_tts_metadata_path(app, tts_path)
                        break
                else:
                    # ?먮낯??李얠쓣 ???놁쑝硫?洹몃?濡?吏꾪뻾 (?꾪뿕)
                    logger.info("[TTS 議곗젙] 二쇱쓽: ?먮낯??李얠쓣 ???놁뼱 議곗젙???뚯씪 ?ъ슜")
            ...
            logger.info("[TTS Adjust] Original length: {audio_duration:.1f}s, target: {target_duration:.1f}s")
            
            # 湲몄씠 李⑥씠媛 1珥??대궡硫?洹몃?濡??ъ슜
            if abs(audio_duration - target_duration) <= 1.0:
                logger.info("[TTS 議곗젙] 湲몄씠媛 ?곸젅??(李⑥씠 {abs(audio_duration - target_duration):.1f}珥?, 洹몃?濡??ъ슜")
                return tts_path
            ...
            if audio_duration > target_duration:
                ...
                logger.info("[TTS 議곗젙] 諛곗냽 ?꾩슂: {speed_ratio:.2f}諛곗냽?쇰줈 {target_duration_with_margin:.1f}珥덈줈 議곗젙")
                
                # ???덉쟾??諛곗냽 泥섎━ ??
                if speed_ratio <= 1.5:
                    # 1.5諛곗냽 ?댄븯: ?덉쭏 ?곗꽑 諛⑹떇
                    logger.info("[TTS 議곗젙] 諛⑸쾿: 怨좏뭹吏?諛곗냽 (pydub effects)")
                    # pydub??effects.speedup ?ъ슜
                    from pydub.effects import speedup
                    adjusted_audio = speedup(audio, playback_speed=speed_ratio, chunk_size=50, crossfade=10)
                elif speed_ratio <= 2.0:
                    # 1.5~2諛곗냽: 以묎컙 ?덉쭏
                    logger.info("[TTS 議곗젙] 諛⑸쾿: 以묎컙 ?덉쭏 諛곗냽 (frame rate 議곗젙)")
                    adjusted_audio = audio._spawn(audio.raw_data, overrides={
                        "frame_rate": int(audio.frame_rate * speed_ratio)
                    }).set_frame_rate(audio.frame_rate)
                else:
                    # 2諛곗냽 珥덇낵: 寃쎄퀬 ??理쒕? 2諛곗냽?쇰줈 ?쒗븳
                    logger.info("[TTS 議곗젙] 寃쎄퀬: {speed_ratio:.2f}諛곗냽? ?덈Т 鍮좊쫫, 2諛곗냽?쇰줈 ?쒗븳")
                    speed_ratio = 2.0
                    adjusted_audio = audio._spawn(audio.raw_data, overrides={
                ...
                logger.info("[TTS Adjust] Speed result: {audio_duration:.1f}s -> {adjusted_duration:.1f}s")
                ...
                logger.info("[TTS Adjust] 배속 후 앞 무음 오프셋: {new_offset:.3f}s")
                _rescale_tts_metadata_to_duration(app, adjusted_duration, new_path=adjusted_path, start_offset=new_offset)
                if hasattr(app, "tts_files"):
                    app.tts_files = [adjusted_path]
                logger.info("[TTS 議곗젙] 諛곗냽 ?뚯씪 ?앹꽦: {adjusted_filename}")
                
            else:
                # TTS媛 吏㏃? 寃쎌슦 ?앹뿉 臾댁쓬 異붽?
                silence_duration = int((target_duration - audio_duration) * 1000)
                logger.info("[TTS Adjust] Added silence: {silence_duration/1000:.1f}s")
                ...
                logger.info("[TTS Adjust] 무음 추가 후 앞 오프셋: {current_offset:.3f}s")
                _rescale_tts_metadata_to_duration(app, adjusted_duration, new_path=adjusted_path, start_offset=current_offset)
                if hasattr(app, "tts_files"):
                    app.tts_files = [adjusted_path]
                logger.info("[TTS 議곗젙] 臾댁쓬 異붽? ?뚯씪 ?앹꽦: {adjusted_filename}")
            
            # ??理쒖쥌 ?뺤씤 ??
            if os.path.exists(adjusted_path):
                final_audio = AudioSegment.from_wav(adjusted_path)
                final_duration = len(final_audio) / 1000.0
                logger.info("[TTS 議곗젙] 理쒖쥌 ?뺤씤: {final_duration:.1f}珥?(紐⑺몴: {target_duration:.1f}珥?")
            
            return adjusted_path
            
        except (OSError, ValueError, RuntimeError) as e:
            logger.error("[TTS Adjust Error] %s", str(e), exc_info=True)
            ui_controller.write_error_log(e)
...
def _update_tts_metadata_path(app, new_path):
    ...
    logger.info("[Subtitles] Updated TTS metadata audio path")


def _rescale_tts_metadata_to_duration(app, total_duration, new_path=None, tolerance=0.001, start_offset=None):
    ...
    if need_scale or need_offset:
        logger.info(
            ...
            current_span, target_span, current_min_start, target_offset
        )
    elif need_path:
        logger.info("[Subtitles] Updated TTS metadata audio path")
...
def _extend_last_subtitle_to_video_end(app, video_duration):
    ...
    logger.info("[WARNING] _extend_last_subtitle_to_video_end is deprecated and should not be called")
    ...
    if original_start >= video_duration:
        ...
        logger.info("[Subtitles] Adjusted last subtitle start from {original_start:.2f}s to {last_entry['start']:.2f}s (was beyond video end)")

    last_entry["end"] = video_duration
    logger.info("[Subtitles] Set last subtitle end to {video_duration:.2f}s (was {original_end:.2f}s) - CTA complete display guaranteed")
...
def create_subtitle_clips_improved(app, video_duration):
    ...
    try:
        ...
        if timed_segments and _metadata_segments_meaningful(timed_segments, video_duration):
            logger.info("[Subtitles] Using {len(timed_segments)} timed segments for burn-in")
            for idx_preview, (start_ts, duration, text) in enumerate(timed_segments[:min(5, len(timed_segments))], start=1):
                end_ts = start_ts + duration
                sample = (text or "").strip()
                if len(sample) > 60:
                    sample = sample[:57] + "..."
                logger.info("[Subtitles] Seg[{idx_preview}] {start_ts:.2f}-{end_ts:.2f}s :: {sample}")
            ...
            if subtitle_clips:
                logger.info("[Subtitles] Applied {len(subtitle_clips)} subtitle clips")
            return subtitle_clips
        elif timed_segments:
            logger.info("[Subtitles] Timed metadata insufficient ({len(timed_segments)} segment), falling back to translation text")

        max_chars = 14
        fallback_segments = _extract_translation_segments(app, desired_count=None, max_chars=max_chars)
        logger.info("[Subtitles] Translation fallback segments: {len(fallback_segments)}")
        if not fallback_segments:
            logger.info("[Subtitles] Translation text missing, skipping subtitle generation.")
            return []

        video_width, video_height = _resolve_video_dimensions(app)

        for idx_preview, text in enumerate(fallback_segments[:min(5, len(fallback_segments))], start=1):
            sample = text.strip()
            if len(sample) > 60:
                sample = sample[:57] + "..."
            logger.info("[Subtitles] Line {idx_preview}: {sample}")
        if not fallback_segments:
            logger.info("[Subtitles] Could not derive any caption segments.")
            return []
        ...
        logger.info("[Subtitles] Error during caption generation: {exc}")
...
def _resolve_video_dimensions(app):
    ...
    if width and height:
        logger.info("[Subtitles] Using cached dimensions {width}x{height}")
    ...
    if source_video and os.path.exists(source_video):
        try:
            ...
            temp_video.close()
            logger.info("[Subtitles] Measured dimensions {video_width}x{video_height} from source")
        except Exception as exc:
            ui_controller.write_error_log(exc)
            logger.info("[Subtitles] Failed to measure video dimensions: {exc}")

    setattr(app, "cached_video_width", video_width)
    setattr(app, "cached_video_height", video_height)
    logger.info("[Subtitles] Cached dimensions set to {video_width}x{video_height}")
...
def _build_timed_subtitle_segments(app, video_duration):
    ...
    logger.info("\n{'='*70}")
    logger.info("[SYNC DEBUG] _build_timed_subtitle_segments 시작")
    logger.info("{'='*70}")
    logger.info("  영상 길이: {video_duration:.3f}초")
    logger.info("  _per_line_tts 개수: {len(per_line) if per_line else 0}")

    # tts_sync_info 출력
    sync_info = getattr(app, "tts_sync_info", {}) or {}
    logger.info("  tts_sync_info:")
    logger.info("    - timestamps_source: {sync_info.get('timestamps_source', 'unknown')}")
    logger.info("    - speeded_duration: {sync_info.get('speeded_duration', 'N/A')}")
    logger.info("    - audio_start_offset: {sync_info.get('audio_start_offset', 'N/A')}")
        ...
        for i, entry in enumerate(per_line):
            if isinstance(entry, dict):
                ...
                if isinstance(start, (int, float)) and isinstance(end, (int, float)):
                    duration = end - start
                    logger.info("    #{i+1}: {start:.3f}s - {end:.3f}s ({duration:.3f}s) '{text_preview}'")
                else:
                    logger.info("    #{i+1}: start={start}, end={end} '{text_preview}'")
            else:
                logger.info("    #{i+1}: [not a dict] {type(entry)}")
    logger.info("{'='*70}\n")
    ...
    if per_line:
        for idx, item in enumerate(per_line, start=1):
            ...
            if not isinstance(item, dict):
                skipped += 1
                logger.info("[Subtitles Skip] Seg #{idx}: Not a dict (type={type(item).__name__})")
                continue
            ...
            if not text or not isinstance(start, (int, float)) or not isinstance(end, (int, float)):
                ...
                logger.info("[Subtitles Skip] Seg #{idx}: Missing/invalid fields: {', '.join(missing_fields)}")
                continue
            ...
            if end <= start:
                if idx == len(per_line):
                    ...
                    safe_start = float(start) if isinstance(start, (int, float)) else 0.0
                    ...
                    logger.info("[Subtitles Adjust] Last segment had non-positive duration -> {safe_start:.2f}-{safe_end:.2f}s")
                    start = safe_start
                    end = safe_end
                else:
                    skipped += 1
                    logger.info("[Subtitles Skip] Seg #{idx}: Invalid timing (start={start:.2f}s >= end={end:.2f}s) - '{text[:20]}'")
                    continue
            ...
            if start_ts >= video_duration + 5.0:  # 5초 이상 초과만 skip (CTA 완전 보호)
                skipped += 1
                logger.info("[Subtitles Skip] Seg #{idx}: Starts too late ({start_ts:.2f}s > video_duration {video_duration:.2f}s + 5.0s) - '{text[:20]}'")
                continue
            ...
            if len(segments) < 3 or len(segments) >= len(per_line) - 3:
                logger.info("[Subtitle Timing] Seg #{len(segments)+1}: {start_ts:.2f}s-{end_ts:.2f}s ({actual_duration:.2f}s) '{text[:20]}'")
            ...
            if start_ts + actual_duration > video_duration:
                ...
                logger.info("[Subtitle Trim] Seg #{len(segments)+1} trimmed: {actual_duration:.2f}s → {duration:.2f}s (video ends at {video_duration:.2f}s)")
            else:
            ...
            if duration < 0.05:  # 0.1에서 0.05로 완화
                skipped += 1
                logger.info("[Subtitles Skip] Seg #{idx}: Too short ({duration:.2f}s < 0.05s) - '{text[:20]}'")
                continue

            # Skip 조건 6: 중복 자막 제거
            # 같은 시작 시간(0.1초 이내)과 같은 텍스트를 가진 세그먼트는 중복으로 간주
            segment_key = (round(start_ts * 10), text)  # 0.1초 단위로 반올림하여 키 생성
            if segment_key in seen_segments:
                duplicates += 1
                logger.info("[Subtitles Skip] Seg #{idx}: Duplicate (start={start_ts:.2f}s, text='{text[:20]}')")
                continue
        ...
        if segments:
            logger.info("\n{'='*70}")
            logger.info("[SYNC DEBUG] 최종 자막 segments - {len(segments)}개")
            logger.info("{'='*70}")
            for i, (start, dur, text) in enumerate(segments):
                end = start + dur
                text_preview = text[:15] if text else f"seg{i}"
                logger.info("    #{i+1}: {start:.3f}s - {end:.3f}s ({dur:.3f}s) '{text_preview}'")
            logger.info("{'='*70}\n")
            ...
            if skipped > 0 or duplicates > 0:
                details = []
                if skipped > 0:
                    details.append("skipped %d" % skipped)
                if duplicates > 0:
                    details.append("duplicates %d" % duplicates)
                summary += " (%s)" % ', '.join(details)
            logger.info(summary)
        else:
            logger.info("[Subtitles] No valid timed segments extracted (skipped %d, duplicates %d)", skipped, duplicates)
...
def _build_analysis_based_segments(app, video_duration):
    ...
    translation_segments = _extract_translation_segments(app, desired_count=len(timeline), max_chars=20)
    ...
    for idx, (start_ts, entry) in enumerate(timeline):
        ...
        next_start = max(start_ts + 0.6, next_start - 0.1 if idx + 1 < total else video_duration)
        ...
        text = translation_segments[idx] if idx < len(translation_segments) else translation_segments[-1]
        text = (text or "").strip()
        if not text:
            continue
    ...
    if segments:
        logger.info("[Subtitles] Built {len(segments)} segments from analysis timestamps")
    return segments


def _extract_translation_segments(app, desired_count=None, max_chars=20):
    """Split translated script into ordered segments suitable for subtitles."""
    # TTS 생성 시 사용된 세그먼트를 그대로 사용 (오디오-자막 싱크 보존)
    if hasattr(app, '_per_line_tts') and app._per_line_tts and isinstance(app._per_line_tts, list):
        ...
        if tts_segments:
            ...
            logger.info("[Subtitles] Using TTS segmentation: {len(tts_segments)} segments (sync preserved)")
            if desired_count:
    ...
    logger.info("[Subtitles] Fallback to translation text: {len(raw)} chars")
    if not raw:
        return []
...
def _map_segments_to_count(segments, target):
    ...
    for idx in range(target):
        ...
        chunk = " ".join(segments[start_idx:end_idx]).strip() if end_idx > start_idx else segments[min(start_idx, total - 1)]
        if not chunk:
...
def _smart_sentence_split(text, max_chars=20):
...
def _split_subtitle_with_gemini(app, text: str, max_chars: int = 10) -> list:
    ...
    try:
        ...
        if not client:
            logger.info("[Subtitle Split] Gemini 클라이언트 없음, 단순 분할 사용")
        ...
        if segments:
            logger.info("[Subtitle Split] Gemini 분할: '{text[:20]}...' -> {len(segments)}개")
            return segments
        else:
            return _fallback_split(text, max_chars)

    except Exception as e:
        logger.info("[Subtitle Split] Gemini 오류: {e}, 단순 분할 사용")
        ui_controller.write_error_log(e)
        return _fallback_split(text, max_chars)
