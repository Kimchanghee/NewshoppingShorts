...
class SubtitleProcessor:
    ...
    def apply_chinese_subtitle_removal(self, video):
        ...
        self.gui.update_progress_state('subtitle', 'processing', 10, "중국어 자막을 분석하고 있습니다.")
        ...
        if hasattr(self.gui, 'apply_blur') and self.gui.apply_blur and not self.gui.apply_blur.get():
            logger.info("[BLUR MAIN] Blur option disabled - skipping")
            self.gui.update_progress_state('subtitle', 'completed', 100, "블러 단계가 비활성화되어 건너뛰었습니다.")
            return video

        try:
            # 이미 분석된 결과가 있으면 재사용
            raw_positions = []
            logger.debug(f"[BLUR MAIN] analysis_result exists: {hasattr(self.gui, 'analysis_result')}")
            if hasattr(self.gui, 'analysis_result'):
                logger.debug(f"[BLUR MAIN] analysis_result content: {self.gui.analysis_result}")

            if hasattr(self.gui, 'analysis_result') and self.gui.analysis_result.get('subtitle_positions'):
                subtitle_positions = list(self.gui.analysis_result.get('subtitle_positions') or [])
                raw_positions = list(self.gui.analysis_result.get('raw_subtitle_positions') or subtitle_positions)
                logger.info(f"[BLUR MAIN] Reusing existing OCR results: {len(subtitle_positions)} regions")
                logger.debug(f"[BLUR MAIN] raw_positions: {len(raw_positions)}")
            else:
                logger.info("[BLUR MAIN] Starting new OCR-based Chinese subtitle detection...")
                ...
                logger.info(f"[BLUR MAIN] OCR detection result: {len(subtitle_positions)} regions")


            detected_positions = raw_positions or subtitle_positions
            logger.debug(f"[BLUR MAIN] Region count before filtering: {len(detected_positions)}")
            for i, pos in enumerate(detected_positions[:10]):  # 처음 10개만 표시
                logger.debug(f"  #{i+1}: x={pos.get('x')}%, y={pos.get('y')}%, w={pos.get('width')}%, h={pos.get('height')}%, text='{str(pos.get('text', ''))[:20]}...'")
            ...
            logger.info(f"[BLUR MAIN] Region count after filtering: {len(chinese_positions)}")
            ...
            if not chinese_positions:
                ...
                logger.debug("  2. Subtitle position is in upper half (y < 50%) and filtered out")
                logger.debug("  3. OCR did not detect text")
                self.gui.update_progress_state('subtitle', 'completed', 100, "중국어 자막이 보이지 않네요. 화면을 깨끗하게 유지합니다.")
                self.gui.update_step_progress('subtitle', 100)
                return video

            self._update_korean_subtitle_layout(chinese_positions)

            logger.info(f"[BLUR MAIN] Processing blur for {len(chinese_positions)} regions")
            for i, pos in enumerate(chinese_positions):
                logger.debug(f"  #{i+1}: x={pos.get('x')}%, y={pos.get('y')}%, w={pos.get('width')}%, h={pos.get('height')}%")
                logger.debug(f"       time={pos.get('start_time', 0):.1f}s~{pos.get('end_time', 'end')}, text='{str(pos.get('text', ''))[:30]}...'")

            self.gui.update_progress_state('subtitle', 'processing', 60, f"중국어 자막 {len(chinese_positions)}개 발견! 블러 처리 중입니다.")
            self.gui.update_step_progress('subtitle', 60)

            w, h = video.size
            blurred_video = self.apply_opencv_blur_enhanced(video, chinese_positions, w, h)

            logger.info("[BLUR MAIN] Blur processing completed")
            logger.debug("=" * 60)
            self.gui.update_progress_state('subtitle', 'completed', 100, f"중국어 자막 {len(chinese_positions)}개 구역 블러 완료!")
            self.gui.update_step_progress('subtitle', 100)
            ...
            self.gui.update_progress_state('subtitle', 'error', 0, "중국어 자막 처리 중 오류가 발생했어요.")
    ...
    def apply_opencv_blur_enhanced(self, video, subtitle_positions, w, h):
        ...
        for idx, pos in enumerate(subtitle_positions):
            ...
            logger.debug(f"    Position: ({final_box[0]}px, {final_box[1]}px) from top-left")
            logger.debug(f"    Size: {blur_width}px x {blur_height}px ({pos['width']:.1f}% x {pos['height']:.1f}%)")
            logger.debug(f"    Time: {start_time:.2f}s ~ {end_time:.2f}s (total {end_time-start_time:.2f}s)")
            ...
            logger.debug(f"  - Original(%): x={pos['x']}%, y={pos['y']}%, w={pos['width']}%, h={pos['height']}%")
            logger.debug(f"  - Original(px): ({x1}, {y1}) ~ ({x2}, {y2})")
            logger.debug(f"  - Padding: pad_x={pad_x}px, pad_y={pad_y}px")
            logger.debug(f"  - Final(px): ({final_box[0]}, {final_box[1]}) ~ ({final_box[2]}, {final_box[3]})")
            logger.debug(f"  - Time range: {start_time:.2f}s ~ {end_time:.2f}s")
            logger.debug(f"  - Text: '{pos.get('text', '')[:30]}...'")

        logger.info(f"[BLUR APPLY] {len(boxes_with_time)} regions ready for time-based blur processing")
        for i, bt in enumerate(boxes_with_time):
            logger.debug(f"  Region {i+1}: {bt['start_time']:.1f}s - {bt['end_time']:.1f}s, box={bt['box']}")
        ...
        feather_size = feather_size + 1 if feather_size % 2 == 0 else feather_size  # 홀수 유지
        ...
        def _feather_blur(get_frame, t):
            ...
            for bt in boxes_with_time:
                ...
                if bt['start_time'] <= t <= bt['end_time']:
                    ...
                    if roi.size == 0:
                        if should_log:
                            logger.debug(f"[BLUR FRAME] t={t:.2f}s - ROI empty, skipping")
                        continue
                    ...
                    frame[Y1:Y2, X1:X2] = (blurred.astype(np.float32)*m3 + roi.astype(np.float32)*(1-m3)).astype(np.uint8)
                    blur_apply_count[0] += 1

            # 매 5초마다 상세 로그 출력
            if should_log:
                if active_boxes:
                    logger.debug(f"[Multi-blur] t={t:.2f}s - Blurring {len(active_boxes)} subtitle regions:")
                    for i, bt in enumerate(active_boxes):
                        X1, Y1, X2, Y2 = bt['box']
                        blur_w = X2 - X1
                        blur_h = Y2 - Y1
                        logger.debug(f"  Subtitle {i+1}: pos=({X1}px, {Y1}px), size={blur_w}px x {blur_h}px, time={bt['start_time']:.1f}~{bt['end_time']:.1f}s")
                else:
                    logger.debug(f"[Multi-blur] t={t:.2f}s - No active blur regions (all outside time range)")
    ...
    def prepare_centered_subtitle_layout(
        ...
        if isinstance(previous, dict):
            previous_signature = (
                previous.get('x'), previous.get('y'),
                previous.get('width'), previous.get('height')
            )
    ...
    def _compute_centered_subtitle_region(
        ...
        def _median_or_default(values: List[float], default: float, minimum: float, maximum: float) -> float:
        ...
        width_pct = _median_or_default(width_candidates, default=60.0, minimum=30.0, maximum=90.0)
        height_pct = _median_or_default(height_candidates, default=18.0, minimum=10.0, maximum=35.0)
    ...
    def _update_korean_subtitle_layout(self, subtitle_positions):
        ...
        try:
            ...
            logger.debug(f'[Korean subtitle] Overlay on Chinese subtitle position: x={avg_x:.1f}%, y={avg_y:.1f}%, w={avg_width:.1f}%, h={avg_height:.1f}%')
