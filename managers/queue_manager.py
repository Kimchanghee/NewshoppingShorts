...
class QueueManager:
    ...
    def clear_url_queue(self):
        ...
        if hasattr(self.gui, "url_timestamps"):
            self.gui.url_timestamps = {
                url: ts
                for url, ts in self.gui.url_timestamps.items()
                if url in processing
            }

        # URL별 상태 메시지도 삭제
        if hasattr(self.gui, "url_status_message"):
            self.gui.url_status_message = {
                url: msg
    ...
    def update_url_listbox(self):
        ...
        processed_items = [
            ...
            if url not in self.gui.url_queue
    ...
    def update_queue_count(self):
        ...
        update_overall_progress_display = getattr(
            self.gui, "update_overall_progress_display", None
        )
    ...
    def paste_and_extract(self, event=None):
        ...
        try:
            ...
            # 텍스트 유효성 검사 (빈 값 또는 None 체크)
            if not clipboard_text or not isinstance(clipboard_text, str):
                show_info(self.gui.root, "안내", "클립보드에 유효한 텍스트가 없습니다.")
                return "break"

        except tk.TclError as e:
            # 클립보드가 비어있거나 접근할 수 없는 경우
            logger.warning("[클립보드] 클립보드 접근 실패 (빈 상태): %s", e)
            show_info(self.gui.root, "안내", "클립보드에 유효한 텍스트가 없습니다.")
            return "break"
        except Exception as e:
            logger.error("[클립보드] 텍스트 가져오기 실패: %s", e)
            show_error(
                self.gui.root, "오류", "클립보드에서 텍스트를 가져오지 못했습니다."
            )
            return "break"
