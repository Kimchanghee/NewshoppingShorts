...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-secrets "DB_PASSWORD=${DB_PASSWORD_SECRET}:latest,JWT_SECRET_KEY=${JWT_SECRET_KEY_SECRET}:latest,ADMIN_API_KEY=${ADMIN_API_KEY_SECRET}:latest,APP_VERSION_UPDATE_API_KEY=${APP_VERSION_UPDATE_API_KEY_SECRET}:latest,APP_VERSION_UPDATE_HMAC_KEY=${APP_VERSION_UPDATE_HMAC_KEY_SECRET}:latest,BILLING_KEY_ENCRYPTION_KEY=${BILLING_KEY_ENCRYPTION_KEY_SECRET}:latest,PAYAPP_USERID=${PAYAPP_USERID_SECRET}:latest,PAYAPP_LINKKEY=${PAYAPP_LINKKEY_SECRET}:latest,PAYAPP_LINKVAL=${PAYAPP_LINKVAL_SECRET}:latest" \
  --min-instances 0 \
  --max-instances 10 \
  --memory 512Mi \
  --cpu 1 \
