...
def test_vercel_postgres_url_is_used_when_database_url_is_not_set(monkeypatch):
    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setenv("POSTGRES_URL", "postgres://user:password@db.example.supabase.co:5432/postgres")
    monkeypatch.setenv("JWT_SECRET_KEY", "a" * 64)
